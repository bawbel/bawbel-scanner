...
from scanner.scanner import scan, _deduplicate as deduplicate, _make_finding
...
class TestGoldenFixture:
    ...
    def test_golden_fixture_scan_time_under_500ms(self):
        ...
        assert result.scan_time_ms < 500, (
            f"Scan took {result.scan_time_ms}ms — Stage 1 must be under 500ms"
        )
...
class TestPatternRulesNegative:
    ...
    def test_clean_web_search_skill(self, tmp_path):
        ...
        result = scan(path)
        assert result.is_clean, (
            f"False positive: {[f.rule_id for f in result.findings]}"
        )
...
class TestScanResult:
    ...
    def test_component_type_detected_from_extension(self, tmp_path):
        for ext, expected in [(".md", "skill"), (".json", "mcp"),
                               (".yaml", "prompt"), (".yml", "prompt")]:
            path = write_skill(tmp_path, f"component{ext}", "# content\n")
            result = scan(path)
            assert result.component_type == expected, (
...
class TestDeduplication:
    ...
    def test_deduplicates_same_rule_id(self):
        findings = [
            Finding("rule-a", None, "Title", "Desc", Severity.HIGH,
                    7.0, 1, "match", "pattern", []),
            Finding("rule-a", None, "Title", "Desc", Severity.CRITICAL,
                    9.0, 2, "match", "yara", []),
        ]
    ...
    def test_keeps_different_rule_ids(self):
        findings = [
            Finding("rule-a", None, "Title A", "Desc", Severity.HIGH,
                    7.0, 1, "match", "pattern", []),
            Finding("rule-b", None, "Title B", "Desc", Severity.HIGH,
                    7.0, 2, "match", "pattern", []),
        ]
        result = deduplicate(findings)
        assert len(result) == 2
...
class TestCLI:
    ...
    def test_cli_fail_on_severity_critical(self):
        ...
        result = runner.invoke(
            cli, ["scan", str(GOLDEN_FIXTURE), "--fail-on-severity", "critical"]
        )
    ...
    def test_cli_fail_on_severity_not_triggered_for_clean(self, tmp_path):
        ...
        runner = CliRunner()
        result = runner.invoke(
            cli, ["scan", path, "--fail-on-severity", "high"]
        )
        assert result.exit_code == 0
...
class TestSecurity:
    ...
    def test_match_length_truncated(self, tmp_path):
        ...
        for finding in result.findings:
            if finding.match:
                assert len(finding.match) <= MAX_MATCH_LENGTH, (
